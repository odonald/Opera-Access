...
def start_server_thread(start):
    ...
    if start:
        ...
        if local_ip == "127.0.0.1":
            ui.lines_and_labels.server_status_label.configure(text=f"LOCAL - No network detected")
            ui.lines_and_labels.server_indicator.configure(bg="red")
        else:
            ui.lines_and_labels.server_status_label.configure(text=f"Live\n http://{local_ip}:{port_number}")
            ui.lines_and_labels.server_indicator.configure(bg="green")
    else:
        server_running = False
        ui.lines_and_labels.server_status_label.configure(text=f"Idle")
        ui.lines_and_labels.server_indicator.configure(bg="red")
...
def change_port():
    ...
    while True:
        new_port = simpledialog.askstring("Change Port", "Enter new port number between 1024 and 65535:", parent=ui.frame_window_setup.root)
        if new_port:
            try:
