...
line_number_var = languages.StringVar()
line_number_entry = ctk.CTkEntry(frame_window_setup.navigation_frame3, placeholder_text="Jump to line")
line_number_entry.grid(row=0, column=0, padx=10, pady=10, sticky="e")

# Create previous, current, and next line labels

progress = ctk.CTkProgressBar(frame_window_setup.root, orientation="horizontal")
progress.grid(row=0, column=1,columnspan=3, padx=0, pady=0, sticky="new")
progress.configure(mode="determinate")
progress.set(0)

navigation_label = ctk.CTkLabel(frame_window_setup.root, text="Display:", font=("", 20))
navigation_label.grid(row=0, column=1, columnspan=3, padx=20, pady=10, sticky="nwe")



prev_line_labels = [ctk.CTkLabel(frame_window_setup.inner_frame, wraplength=400, text="---") for _ in range(5)]

current_line_label = ctk.CTkLabel(frame_window_setup.inner_frame, text_color=("Yellow", "#FFD90F"), text="Please import a language or load a session.\n +\n <--- Choose display language", font=("", 25))
current_line_label.grid(row=1, column=0, padx=10, pady=10)

next_line_labels = [ctk.CTkLabel(frame_window_setup.inner_frame, wraplength=400, text="---") for _ in range(20)]
...
def update_label():
    ...
    if additional_languages:
        ...
        selected_language = languages.language.get()

        lang_code = languages.available_languages[selected_language]
...
def combine_files():
    ...
    if languages.original_lines and languages.translation_lines:
        combined_lines = list(zip(languages.original_lines, languages.translation_lines))
    else:
        combined_lines = [(line, "") for line in languages.original_lines]
    update_label()
...
def set_current_line(line):
    ...
    if current_line_clicks == 1:
        languages.send_to_server(empty_line)
    ...
    if current_line_clicks == 2:
        ...
        languages.send_to_server(current_line)  # Send the clicked line to the server
        update_label()
...
def next_line():
    ...
    if current_line is not None and current_line < len(additional_languages[lang_code]) - 1:
        ...
        if next_button_clicks == 2:
            ...
            current_line_clicks = 0
            update_label()
            languages.send_to_server(current_line)
    else:
        next_button_clicks = 0
...
def next_line():
    ...
    if current_line is not None and current_line < len(additional_languages[lang_code]) - 1:
        next_button_clicks += 1
        if next_button_clicks == 1:
            languages.end_to_server(empty_line)
